...
ParamonovIConstrHullSeq::ParamonovIConstrHullSeq(const InType &in) : work_(in) {
  SetTypeOfTask(GetStaticTypeOfTask());
  GetInput() = in;
}

bool ParamonovIConstrHullSeq::ValidationImpl() {
...
bool ParamonovIConstrHullSeq::PreProcessingImpl() {
  work_ = GetInput();
  ThresholdImage();
  return true;
}

bool ParamonovIConstrHullSeq::RunImpl() {
  FindComponents();
...
bool ParamonovIConstrHullSeq::PostProcessingImpl() {
  return true;
}

size_t ParamonovIConstrHullSeq::Index(int x, int y, int width) {
  return (static_cast<size_t>(y) * static_cast<size_t>(width)) + static_cast<size_t>(x);
}

void ParamonovIConstrHullSeq::ThresholdImage() {
  for (auto &p : work_.pixels) {
    p = IsForeground(p) ? static_cast<uint8_t>(255) : static_cast<uint8_t>(0);
  }
}

void ParamonovIConstrHullSeq::ExploreComponent(int start_col, int start_row, int width, int height,
...
void ParamonovIConstrHullSeq::FindComponents() {
...
std::vector<Point> ParamonovIConstrHullSeq::BuildHull(const std::vector<Point> &points) {
  if (points.size() <= 2) {
    return points;
  }
