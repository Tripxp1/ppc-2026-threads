...
const auto kSeqTestTasksList = std::tuple_cat(
...
const auto kSeqFuncTestName = ConvexHullFuncTest::PrintFuncTestName<ConvexHullFuncTest>;

INSTANTIATE_TEST_SUITE_P(ParamonovHullSeqTests, ConvexHullFuncTest, kSeqGtestValues, kSeqFuncTestName);

const auto kOmpTestTasksList = std::tuple_cat(
    ppc::util::AddFuncTask<ConvexHullOMP, InputType>(kTestCases, PPC_SETTINGS_paramonov_v_bin_img_conv_hul));

const auto kOmpGtestValues = ppc::util::ExpandToValues(kOmpTestTasksList);

const auto kOmpFuncTestName = ConvexHullFuncTest::PrintFuncTestName<ConvexHullFuncTest>;

INSTANTIATE_TEST_SUITE_P(ParamonovHullOmpTests, ConvexHullFuncTest, kOmpGtestValues, kOmpFuncTestName);
