...
const auto kSeqPerformanceTasks =
    ppc::util::MakeAllPerfTasks<InputType, ConvexHullSequential>(PPC_SETTINGS_paramonov_v_bin_img_conv_hul);

const auto kSeqTestValues = ppc::util::TupleToGTestValues(kSeqPerformanceTasks);

INSTANTIATE_TEST_SUITE_P(ParamonovPerfSeqTests, ConvexHullPerformanceTest, kSeqTestValues,
                         ConvexHullPerformanceTest::CustomPerfTestName);

const auto kOmpPerformanceTasks =
    ppc::util::MakeAllPerfTasks<InputType, ConvexHullOMP>(PPC_SETTINGS_paramonov_v_bin_img_conv_hul);

const auto kOmpTestValues = ppc::util::TupleToGTestValues(kOmpPerformanceTasks);

INSTANTIATE_TEST_SUITE_P(ParamonovPerfOmpTests, ConvexHullPerformanceTest, kOmpTestValues,
                         ConvexHullPerformanceTest::CustomPerfTestName);
